...
function Calc() {
	...
	if ((Player_ATK + WeaponATK) >= ATK_Limit) {
	...
	else if((Player_ATK + CrtWeaponATK) >= ATK_Limit){
		let Avg_temp =
		Math.round((Player_ATK + CrtWeaponATK - ATK_Limit) / (CrtWeaponATK - WeaponATK));
		let Min_Result_temp =
		...
		let Avg_Result_temp =
			Math.round((parseFloat(1) - CrtRate) * Avg_temp * Max_Result_temp + (parseFloat(1) - CrtRate) * (1 - Avg_temp) * (Max_Result_temp + Min_Result_temp) / 2 + CrtRate * Crt_Result_temp);
		document.getElementById('Avg_Result').value = Avg_Result_temp;
	}
	else {
		let Min_Result_temp =
